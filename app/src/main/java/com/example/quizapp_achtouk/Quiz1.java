...
public class Quiz1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView1 = findViewById(R.id.qst1);
        imageView = findViewById(R.id.image);
        score = getIntent().getIntExtra("score", 0);
        num_qst = getIntent().getIntExtra("num_qst", 1);
        textView2 = findViewById(R.id.quiz);
        ...
        db.collection("QuizApp")
                .document("qpv68PaRl0eTtQXp00i7")
                .get()
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        DocumentSnapshot document = task.getResult();
                        if (document.exists()) {
                            RepCorrect = document.getString("Question" + num_qst + ".Reponse");
                            List<String> choices = (List<String>) document.get("Question" + num_qst + ".Choix");
                            String question = document.getString("Question" + num_qst + ".Question");

                            textView2.setText("Quiz " + num_qst);
